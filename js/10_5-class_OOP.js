...
// // ? или class Car{}; - короткое объявление класа // const carInstance = new Car(); и его экземпляра
...
// //todo или так !!! ЗНАЧИТЕЛЬНО ПРОЩЕ (сразу в prototype и без ;)
class Car {
    static description = 'Класс описывающий автомобиль'; // статическое свойство класса доступное только для класса
    #test = 'test приватное свойство класса Car'

    constructor({ brand, model, price } = {}) { // constructor вызывается автоматически и сразу в prototype
        this.brand = brand;
        this.model = model;
        this.price = price;
    }
...
// //* get, set также работают и с объектами
// const obj = {
//     _a: 100,
//     _b: 300,
//     get a() {
...
//     },
// };
// console.log(obj); // {_a: 100, _b: 300}
// console.log(obj.a); // сеттер для а  100
// obj.a = 444; // геттер для а
// console.log(obj); // {_a: 444, _b: 300}
...
//todo ========================= НАСЛЕДОВАНИЕ КЛАССОВ ОТ РЕПЕТЫ ====================================================

class Hero { // * создали основной (родительский класс)
...
class Warrior extends Hero {  // * создали(дочерний класс) с наследованием у  класса Hero с помощью extends
    constructor({weapon, ...rest}) {
        super(rest); //* нужно вызвать constructor у Hero с помощью super
        this.weapon = weapon;
    }
}

class Berserk extends Warrior {  // * создали(дочерний класс) с наследованием у  класса Warrior с помощью extends
    constructor({spells, ...rest}) {
        super(rest); //* нужно вызвать constructor у Hero с помощью super
        this.spells = spells;
    }
    magic() {
        console.log(`${this.name} делает заклинание с помощью ${this.spells}`);
    }
}

const havkai = new Warrior({name: 'Andrii',xp: 1000,weapon: 'лук'}); //* создали ЄК havkai класса Warrior
console.log(havkai); // Warrior {name: 'Andrii', xp: 1000, weapon: 'лук'}
havkai.gainXp(500); // Andrii получает 500 опыта
const mangalur = new Berserk({ name: 'Maks', xp: 1000, weapon: 'кинжал', spells: 'чума' }); //* создали ЄК mangalur класса Berserk
console.log(mangalur);
mangalur.magic(); // Maks делает заклинание с помощью чума